...
uint8 MCAL_I2C_masterTx(uint16 slaveAddress, uint8 *dataTx, uint32 dataLength,
	...
	// 2. I2CNT set. Status code indicates START condition sent
	I2C_generateStart();
	...
	// making sure that I2CNT is written to one, and TWSTA is written to zero
	I2C_sendAddress(slaveAddress, I2C_DIRECTION_TRANSMITTER);

	// 4. I2CNT set. Status code indicates SLA+W sent, ACK received
	...
	// making sure that I2CNT is written to one
	...
	for (i = 0; i < dataLength; i++) {
		...
		// 6. I2CNT set. Status code indicates data sent, ACK received
	...
	// into TWCR, making sure that I2CNT is written to one
	// Stop Condition
...
uint8 MCAL_I2C_masterRx(uint16 slaveAddress, uint8 *dataRx, uint32 dataLength,
	...
	// 4. I2CNT set. Status code indicates SLA+R sent, ACK received
	// 5. Check TWSR to see if SLA+W was sent and ACK received. Application
	// loads data into TWDR, and loads appropriate control signals into TWCR,
	// making sure that I2CNT is written to one
	...
	for (i = 0; i < dataLength; i++) {
		...
		// 6. I2CNT set. Status code indicates data received, ACK sent
...
void I2C_generateStart(void) {
	...
	// Immediately after the application has cleared I2CNT,
...
ISR(TWI_vect) {
	// Check for flags
	// Bit 7 I2CNT: I2C Interrupt Flag
	//The I2CNT Flag must be cleared by software by writing a logic one to it. Note that this flag is not
